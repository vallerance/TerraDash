...
def ensure_population(c,cache:Path,year:int):
    ensure_base(c,cache)
    key=f"population_{year}_1km"
    if done(c,key):
        return
    raster,ver=get_worldpop(cache,year)
    from shapely import from_wkb

    progress_dir=cache/"progress"/"population"
    ...
    with rasterio.open(raster) as ds:
        ...
        for pos,(_,window) in enumerate(ds.block_windows(1)):
            if pos < completed:
                continue
            ...
            if arr.count():
                ...
                rows=c.execute(
                    "SELECT g.island_id,g.wkb FROM island_geometry_rtree r JOIN island_geometry g ON g.island_id=r.island_id WHERE r.maxx>=? AND r.minx<=? AND r.maxy>=? AND r.miny<=?",
                ...
                if rows:
                    ...
                    if shapes:
                        ...
                        if valid.any():
                            binc=np.bincount(labels[valid].ravel(),weights=values[valid].ravel(),minlength=max_id+1)
                            totals[:len(binc)] += binc
            totals.flush()
            state_path.write_text(str(pos+1))
            if (pos+1)%100==0:
                print(f"WorldPop blocks: {pos+1}",flush=True)

    updates=[]
    for iid in range(1,max_id+1):
        val=float(totals[iid])
        if val!=0 or c.execute("SELECT 1 FROM islands WHERE id=?",(iid,)).fetchone():
            updates.append((val,year,ver,"1km-raster",iid))
        if len(updates)>=10000:
            c.executemany("UPDATE islands SET population=?,population_year=?,population_source=?,population_method=? WHERE id=?",updates)
            c.commit(); updates=[]
    if updates:
        c.executemany("UPDATE islands SET population=?,population_year=?,population_source=?,population_method=? WHERE id=?",updates)
    c.execute("CREATE INDEX IF NOT EXISTS idx_islands_population ON islands(population)")
    c.commit()
    mark(c,key,ver)
