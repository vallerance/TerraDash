...
for (const viewport of [
  ...
  test(`Canadian Provinces full quiz ${viewport.name}`, async ({
    page,
  }, testInfo) => {
...
test('Prince Edward Island magnifier topology', async ({ page }, testInfo) => {
  ...
  await page.screenshot({
    path: testInfo.outputPath('canadian-pei-magnifier-mobile.png'),
    fullPage: true,
  });
});

test('Newfoundland and Labrador multipart coastal topology', async ({
  page,
}, testInfo) => {
  ...
  await page.screenshot({
    path: testInfo.outputPath('canadian-newfoundland-labrador-multipart.png'),
    fullPage: true,
  });
});

test('Canadian quiz normal answer and advance flow', async ({
  page,
}, testInfo) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto(`${base}?quiz=canadian-provinces&start=1`);
  const target = page
    .locator('.world-map .active-fill path[data-location-id]')
    .first();
  ...
  await page.screenshot({
    path: testInfo.outputPath('canadian-answer-and-advance.png'),
    fullPage: true,
  });
});
